...
export async function getAndBuildAnswer(message, user) {
  ...
  await updateUserAnimal(message, user)
...
async function updateUserAnimal(message, user){
  ...
  if(message.quick_reply){
    ...
    if(payload.id === ID_ANSWER_LAPIN){
      await Users.update({_id: user._id}, {$set: {'animals.0.species' : "lapin"}}) // await pas nécessaire
    }
    if(payload.id === ID_ANSWER_CHIEN){
      await Users.update({_id: user._id}, {$set: {'animals.0.species' : "chien"}}) // await pas nécessaire
    }
  }
}
