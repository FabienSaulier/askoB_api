 ...
 async function addTransitionToOtherSpeciesAnswers(answers, entitiesValues, currentSpecies){
  if(_.includes(entitiesValues, "ESPECE_NL") || _.includes(entitiesValues, "ESPECE_NC") || _.includes(entitiesValues, "ESPECES_NC")){
    ...
    for(let species of species){
      ...
      a.quickReplyLabel_fr = "Changer d'espèces "+emoji(species)
      answers.push(a)
    }
  }
  return answers
 ...
 * @param  {type} user
 */
export async function updateUserQuestionSpecies(event, user){
  async function update(answerID){
    switch (answerID) {
  ...
  if(_.has(event, 'postback.payload'))
    update(event.postback.payload)
  if(_.has(event, 'message.quick_reply.payload')){
