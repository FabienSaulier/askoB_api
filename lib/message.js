 ...
 * @return Answer
 */
export async function findAnswer(species, intent, entitiesAndValues, passageCount = 1) {
  ...
  if (areSeveral(answers)) {
    ...
    if (answersFiltered.length > 1) { return answersFiltered }
  }

  if (canExploseEntities(entitiesAndValues[0])) { return findAnswer(species, intent, exploseArrays(entitiesAndValues), passageCount + 1) }
