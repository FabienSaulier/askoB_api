...
export function analyseMessage(message, species) {
  ...
  logger.info('receivedMessage - bot '+species+' : ', JSON.stringify(messageText))
  ...
  if(species === 'chien')
    ...
    clientRecast = new Recastai(config.RECAST_DEV_ACCESS_TOKEN_CHAT)
  else
 ...
 * @return Answer
 */
export async function findAnswer(species, entitiesAndValues) {
  if(entitiesAndValues[0].length === 1){
    if(entitiesAndValues[0][0] === 'GREETINGS' || entitiesAndValues[0][0] === 'GREETING' ) {return findAnswer(species, [['THEMES']])}
    if(entitiesAndValues[0][0] === 'GOODBYES') { return [Answers.findOneRandomByIntent('goodbye')] }
    if(entitiesAndValues[0][0] === 'INSULTE') { return [Answers.findOneRandomByIntent('insult')] }
  }
  ...
  if (canExploseEntities(entitiesAndValues[0])) { return findAnswer(species, exploseArrays(entitiesAndValues)) }

  return  await getNoAnswerFound(species)
}
...
async function getNoAnswerFound(species){
  ...
  if(species === undefined || species == ''){
    return [await Answers.findOne({_id: ANSWERS_ID.ANSWER_NO_SPECIES_SELECTED_ID})]
  } else if(species === 'autres'){
    return [answer.push(Answers.dontUseNLP)]
  } else
  return [await Answers.findOneRandomByIntent('unknow')]
}
