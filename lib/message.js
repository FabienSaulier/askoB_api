...
export async function findAnswer(intent, entitiesAndValues, passageCount){
  ...
  const answers = await Answers.find({ entities:{ $all:entitiesAndValues}})
...
function explose(entitiesAndValues){
  let explosedEntities = exploseWithDuplicates(entitiesAndValues)
  explosedEntities = removeDuplicates(entitiesAndValues)
  return explosedEntities
}

function exploseWithDuplicates(entitiesAndValues){
  let explodedWithDuplicated = []
  entitiesAndValues.forEach(
    (e)=>{
      entitiesAndValues.forEach(
        (f)=>{
          explodedWithDuplicated.push([e,f])
        }
      )
    }
  )
...
function removeDuplicates(entitiesAndValues){
