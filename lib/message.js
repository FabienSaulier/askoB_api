      ...
      .then(function(result) {
        ...
        .then((entitiesValues) => {
          ...
          if(intent && intent.slug == 'greetings')
            ...
            sendSpecificAnswer(entities, entitiesValues, senderID, false);
          else
            sendDefaultAnswer(senderID);
...
export function sendSpecificAnswer(entities, entitiesValues, senderID, secondPass){
  if(secondPass){
  ...
  query.then(
    function(answers){
      ...
      const constPerfectAnswer = getAnswerWhichMatchEntitiesValues(answers, entities, entitiesValues)
      if(constPerfectAnswer){
        ...
        const generalAnswer = getAnswerWhichMatchEntitiesValues(answers, entities, entitiesValues)
        if(generalAnswer){
          ...
          if(secondPass)
            ...
            removeFilteredEntities(entities)
              .then((ent) => {
                sendSpecificAnswer(ent, entitiesValues, senderID, true)
              })
...
function getAnswerWhichMatchEntitiesValues(answers, entities, entitiesValues){
