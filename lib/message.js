...
export async function analyseMessage(message, species) {
  ...
  messageText = await translate(messageText)
 ...
 */
async function translate(messageText){
  const translate = new Translate({
    key: config.G_TRANSLATE_API_KEY
  });
  const targetLanguage = 'fr'
  const results = await translate.translate(messageText, targetLanguage)
  const translation = results[0];
  if(messageText !== translation){
    logger.info(`Text: ${messageText}  =>  ${translation}`)
