...
export async function findAnswer(species, entitiesAndValues) {
  ...
  console.log("entitiesAndValues ",entitiesAndValues)
  if(entitiesAndValues[0].length === 1){
    if(entitiesAndValues[0][0] === 'GREETINGS') { return Answers.findOneRandomByIntent('greetings') }
    if(entitiesAndValues[0][0] === 'GOODBYES') { return Answers.findOneRandomByIntent('goodbye') }
    if(entitiesAndValues[0][0] === 'INSULTS') { return Answers.findOneRandomByIntent('insult') }
  }
