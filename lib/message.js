...
export async function findAnswer(intent, entitiesAndValues, passageCount = 1) {
  ...
  if (areSeveral(answers)) {
    const dimension = entitiesAndValues.length - (passageCount + 1) // 1 is the modifier for array
  ...
  if (canExploseEntities(entitiesAndValues[0]))
    return findAnswer(intent, exploseArrays(entitiesAndValues), passageCount + 1)
