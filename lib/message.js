 ...
 *
 * @param  {string} intent
 * @param  {array[array]} entitiesAndValues
...
/**
 * explose - explose an array into severals
 * ex: ['A','B','C'] => [['A','B'],['A','C'],['B','C']]
 *
 * @param  {array} entities, entities.length > 1
 * @return {array[array]}
 */
...
function removeDuplicates(explodedEntities){
  const minLength = explodedEntities.pop().length
  ...
  const withoutDuplicates = _array.uniqWith(entitieswithDuplicates, _lang.isEqual)
  logger.info('res',withoutDuplicates)
